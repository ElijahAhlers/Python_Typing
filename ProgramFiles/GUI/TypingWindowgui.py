...
class TypingWindow(Screen):
    ...
    def recordResults(self):
        file = self.manager.location+'UserData/'+self.manager.user.username+'/History'
        ...
        if fileExists(file):
            ...
            writeNewCSVFile(file, ['Lesson', 'Accuracy', 'WPM', 'Idle Time'],
                            [{'Lesson': self.manager.lesson.filename,
                              'Accuracy': self.accuracy,
                              'WPM': self.realWPM,
                              'Idle Time': self.fileIdleTime}])
        self.manager.resultsObject.addResults(
            lessonName=self.manager.lesson.filename,
            accuracy=self.accuracy,
            wpm=self.realWPM,
        )
    ...
    def Exit(self):
        ...
        self.lesson_clock.cancel()
        self.recordResults()
        self.manager.current = 'ResultsWindow'
        self.manager.get_screen('ResultsWindow').update_everything()
